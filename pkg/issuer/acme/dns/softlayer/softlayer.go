...
func (c *DNSProvider) Present(domain, token, key string) error {
	fqdn, value, ttl, err := util.DNS01Record(domain, key, c.dns01Nameservers, true)
	if err != nil {
		return err
	}
	...
	chg, err := svc.Id(*zone).CreateTxtRecord(&entry, &value, &ttl)
	if err != nil {
		fmt.Println(chg.Domain)
		return err
	}
...
func (c *DNSProvider) CleanUp(domain, token, key string) error {
	fqdn, _, _, err := util.DNS01Record(domain, key, c.dns01Nameservers, true)
	if err != nil {
		return err
	}
...
func (c *DNSProvider) getHostedZone(domain string) (*int, error) {
	zone, err := util.FindZoneByFqdn(util.ToFqdn(domain), util.RecursiveNameservers)
	if err != nil {
		return nil, err
	}

	svc := services.GetDnsDomainService(c.session)
	softlayerZone := util.UnFqdn(zone)
	zones, err := svc.GetByDomainName(&softlayerZone)
	if err != nil {
		return nil, fmt.Errorf("Softlayer API call failed: %v", err)
	}

	if len(zones) == 0 {
		return nil, fmt.Errorf("No matching Softlayer domain found for domain %s", softlayerZone)
	}
...
func (c *DNSProvider) findTxtRecords(zone int, entry string) ([]datatypes.Dns_Domain_ResourceRecord, error) {
	...
	// Look for existing records.
	svc := services.GetDnsDomainService(c.session)
	recs, err := svc.Id(zone).GetResourceRecords()
	if err != nil {
		return nil, err
	}
